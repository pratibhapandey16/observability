...
const renderLogsView = () => {
  ...
  cy.get('[data-test-subj="configPane__vizTypeSelector"] [data-test-subj="comboBoxInput"]').type('Logs view').type('{enter}');
  cy.get('#configPanel__panelOptions .euiFieldText').click().type('Logs view');
  cy.get('.euiFlexItem .euiFormRow [placeholder="Description"]').click().type('This is the description for Logs view');
  cy.get('.visEditorSidebar__config .euiIEFlexWrapFix').eq(1).contains('Chart styles').should('exist');
  cy.get('#configPanel__chart_styles .euiFormLabel.euiFormRow__label').contains('Time');
  cy.get('#configPanel__chart_styles .euiFormLabel.euiFormRow__label').contains('Wrap lines');
  cy.get('#configPanel__chart_styles .euiFormLabel.euiFormRow__label').contains('Prettify JSON');
  cy.get('#configPanel__chart_styles .euiFormLabel.euiFormRow__label').contains('Enable log details');
  cy.get('#configPanel__chart_styles .euiTitle.euiTitle--xxsmall').eq(0).contains('Order');
  cy.get('[data-test-subj="newestFirst"]').should('be.checked');
  cy.get('#configPanel__chart_styles .euiTitle.euiTitle--xxsmall').eq(1).contains('Label Size').type('25');
...
describe('Render Logs view', () => {
  beforeEach(() => {
    renderLogsView();
  });

  it.only('Render Logs view, Save and Delete Visualization', () => {
    saveVisualizationAndVerify();
    cy.wait(delay * 4);
    deleteVisualization();
  });

  it('Table view disabled for Logs view', () => {
    cy.get('[data-test-subj="workspace__dataTableViewSwitch"]').contains('Table view').should('be.disabled');
  });
});
