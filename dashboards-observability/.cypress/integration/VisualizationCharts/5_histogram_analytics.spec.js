...
const renderHistogramChart = () => {
  ...
  cy.get('.euiAccordion__button[aria-controls="configPanel__Color Theme"]').contains('Color Theme').should('exist');
  cy.get('[aria-controls="configPanel__legend"]').contains('Legend');
  cy.get('#configPanel__legend').contains('Show Legend');
  cy.get('.euiButton__text.euiButtonGroupButton__textShift[data-text="Show"]').should('not.be.disabled');
  cy.get('#configPanel__legend').contains('Position');
  cy.get('.euiButton__text.euiButtonGroupButton__textShift[data-text="Right"]').should('not.be.disabled');
};
...
describe('Renders Histogram chart', () => {
  ...
  it.only('Renders Histogram chart and Data Configurations panel', () => {
   cy.get('.euiResizablePanel.euiResizablePanel--collapsible.euiResizablePanel--middle')
   ...
   cy.get('.euiPanel.euiPanel--paddingSmall.euiPanel--borderRadiusMedium.euiPanel--plain.euiPanel--hasShadow.dataConfigContainer').click();   
  });
});
